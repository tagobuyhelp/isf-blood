...
export const updateAvatar = async (req, res, next) => {
    try {
        ...
        // Optionally delete previous avatar from Cloudinary
        try {
            const previous = await User.findById(userId).select('avatar_public_id');
            if (previous?.avatar_public_id) {
                const { deleteFromCloudinary } = await import('../utils/fileUpload.js');
                await deleteFromCloudinary(previous.avatar_public_id);
            }
        } catch (err) {
        ...
        const user = await User.findByIdAndUpdate(
            userId,
            { avatar: result.url, avatar_public_id: result.public_id },
            { new: true }
        ).select('-password');
...
/**
 * @desc    Get donation history for the authenticated user (as donor)
...
export const getDonationHistory = async (req, res, next) => {
    try {
        ...
        if (!donor) {
            // Return empty list to simplify frontend handling when donor profile doesn't exist
            return res.status(200).json(new ApiResponse(200, { donations: [] }, 'Donation history retrieved'));
        }
        ...
        res.status(200).json(new ApiResponse(200, { donations, lastDonationDate: donor.lastDonationDate || null }, 'Donation history retrieved'));
    } catch (error) {
        next(error);
    }
};
...
/**
 * @desc    Add a donation entry (date, hospital, units) to the authenticated user's donor profile
 * @route   POST /api/v1/users/donations
 * @access  Private
 */
export const addDonationEntry = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { date, hospital, units } = req.body;

        if (!date || !hospital || units === undefined || units === null) {
            return next(new ApiError(400, 'date, hospital, and units are required'));
        }
        ...
        if (!Number.isFinite(unitsNum) || unitsNum < 1 || unitsNum > 10) {
            return next(new ApiError(400, 'units must be a number between 1 and 10'));
        }
        ...
        donor.donationHistory.push({ date: parsedDate, hospital: String(hospital).trim(), units: unitsNum });

        // Update lastDonationDate to the latest donation date
        const latest = donor.donationHistory.reduce((acc, d) => {
            const t = new Date(d.date).getTime();
            return t > acc ? t : acc;
        }, donor.lastDonationDate ? new Date(donor.lastDonationDate).getTime() : 0);
        donor.lastDonationDate = latest ? new Date(latest) : donor.lastDonationDate;

        await donor.save();

        const donations = donor.donationHistory.slice().sort((a, b) => new Date(b.date) - new Date(a.date));
        res.status(201).json(new ApiResponse(201, { donations, lastDonationDate: donor.lastDonationDate || null }, 'Donation added'));
    } catch (error) {
        next(error);
    }
};