...
/**
 * @desc    Social login with Google using ID token
...
export const googleLogin = async (req, res, next) => {
    try {
        ...
        if (!clientId) {
            return next(new ApiError(500, 'Google client ID not configured'));
        }
        ...
        if (!email) {
            return next(new ApiError(400, 'Google account does not provide an email'));
        }

        // Find or create the user
        let user = await User.findOne({ email });
        if (!user) {
            // Create a random password to satisfy model requirements
            ...
            // Ensure email verified flag is updated
